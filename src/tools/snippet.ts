  ...
  server.registerTool(
    ...
    async ({ moduleName }) => {
      ...
      try {
        ...
        throw new Error(
          `Failed to load snippet for module ${moduleName}: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
  );
