 ...
 */
export function registerModuleDetailTool(server: McpServer) {
  server.registerTool(
    'acms_get_module_detail',
    {
      title: 'Get module detail from API',
      description:
        'a-blog cmsのAPIからモジュールの詳細情報（変数、パラメータ、設定項目など）を取得します。スニペットに含まれていない変数や詳細なパラメータ情報も含まれます。acms_get_snippetと組み合わせて使用することで、完全なコード生成が可能になります。moduleIdには、acms_modulesで取得した一覧のidentifierフィールドの値を使用してください。',
      inputSchema: {
        moduleId: z
          .string()
          .describe(
            'モジュールID（acms_modulesで取得した一覧のidentifierフィールドの値。例: entry-list, news-list, news-detail）'
          ),
      },
    },
    async ({ moduleId }) => {
      ...
      try {
        ...
        throw new Error(
          `Failed to load module detail for ${moduleId}: ${errorMessage}. ` +
